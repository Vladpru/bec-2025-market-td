...
SPREADSHEET_TITLE = 'TD market transactions' # ВАЖЛИВО: вкажіть точну назву вашої таблиці
...
async def log_action(action: str, user_id: int = None, details: str = ""):
    """
    Асинхронно записує рядок логу в Google Sheets.
    """
    timestamp = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    data_to_log = [timestamp, action, str(user_id) if user_id else "N/A", details]
